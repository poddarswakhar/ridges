...
class TemperatureAutoController:
    ...
    def record_performance(self, success: bool, response_quality: float = None, error_type: str = None):
        ...
        if response_quality is not None:
            ...
            self.avg_response_quality = ((self.avg_response_quality * (
                        self.response_count - 1)) + response_quality) / self.response_count
    ...
    def _adjust_temperature(self):
        ...
        if self.consecutive_errors >= 3:
            if self.current_temp < self.max_temp - 0.005:  # Leave small buffer
                ...
                logger.info(
                    f"Temperature increased to {self.current_temp:.4f} due to {self.consecutive_errors} consecutive errors")
        ...
        # Rule 2: Many consecutive successes -> decrease temperature for more consistency
            ...
            if self.current_temp > self.min_temp + 0.005:  # Leave small buffer
                ...
                adjustment_made = True
                logger.info(
                    f"Temperature decreased to {self.current_temp:.4f} due to {self.consecutive_successes} consecutive successes")
        ...
        # Rule 3: Low response quality -> increase temperature
            ...
            if self.current_temp < self.max_temp - 0.005:
                self.current_temp = min(self.max_temp,
                                        self.current_temp + self.adjustment_factor * 2.0)  # Larger adjustment for quality issues
                adjustment_made = True
                logger.info(
                    f"Temperature increased to {self.current_temp:.4f} due to low response quality ({self.avg_response_quality:.3f})")
        ...
        # Rule 4: High response quality but recent errors -> fine-tune temperature
            ...
            if len(recent_errors) > 2:
                self.current_temp = max(self.min_temp,
                                        min(self.max_temp, self.current_temp + self.adjustment_factor * 0.5))
                adjustment_made = True
                logger.info(
                    f"Temperature fine-tuned to {self.current_temp:.4f} due to recent errors despite good quality")
...
class ParallelFileSearcher:
    ...
    def search_multiple_files_parallel(self, search_terms: List[str], file_patterns: List[str] = None) -> Dict[
        str, str]:
...
class COT:
    ...
    def export_to_csv(self, file_path: str = "./xray.csv"):
        with open(file_path, "w") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["next_thought", "next_tool_name", "next_tool_args", "observation", "is_error", "raw_response",
                 "total_attempts", "is_deleted"])
            if len(self.thoughts) > 0:
                for thought in self.thoughts:
                    writer.writerow(
                        [thought.next_thought, thought.next_tool_name, thought.next_tool_args, thought.observation,
                         thought.is_error, thought.raw_response, thought.total_attempts,
...
class Network:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Temperature Auto-Controller initialized with temperature: {self.temp_controller.get_current_temperature():.4f} (Range: 0.0000-0.1000)")
    ...
    def make_request(cls, messages: list, attempt: int = 10) -> str:
        ...
        is_oai_interface = type(response_json) is dict and response_json.get('choices') is not None and len(
            response_json.get('choices')) > 0 and response_json.get('choices')[0].get('message') is not None
        if is_oai_interface:
    ...
    def parse_next_tool_args(cls, tool_name: str, next_tool_args: str) -> dict | str:
        ...
        try:
            ...
            try:
                next_tool_args = cls.parse_malformed_json(ToolManager.get_tool_args_for_tool(tool_name, required=True),
                                                          next_tool_args)
    ...
    def sanitise_text_resp(cls, text_resp: str) -> str:
        ...
        if "next_thought" not in text_resp and "next_tool_name:" in text_resp and "next_tool_args:" in text_resp and text_resp.find(
                "next_tool_name:") < text_resp.find("next_tool_args:") and text_resp.find("next_tool_name:") > 10:
        ...
        if "next_tool_name:" in text_resp and "next_tool_args:" in text_resp and text_resp.find(
                "next_tool_name:") < text_resp.find("next_tool_args:"):
            # remove all leading and trailing quotes in tool_name
            next_tool_name = text_resp.split("next_tool_name:")[1].split("next_tool_args:")[0].strip().strip(
                "\n").strip("\'").strip("\"").strip()
            print(text_resp)
            text_resp = re.sub(f"next_tool_name:[\'\" ]*{next_tool_name}[\'\" ]*", "next_tool_name: " + next_tool_name,
                               text_resp)
    ...
    def parse_response(cls, text_resp: str) -> tuple[str, str, dict]:
        ...
        if "next_thought:" in text_resp and "next_tool_name:" in text_resp and "next_tool_args:" in text_resp and text_resp.find(
                "next_thought:") < text_resp.find("next_tool_name:") and text_resp.find(
                "next_tool_name:") < text_resp.find("next_tool_args:"):
...
```
    ...
    def get_tool_docs(cls) -> str:
        return '\n\n'.join(
            [json.dumps(tool_metadata, ensure_ascii=False) for _, tool_metadata in cls.TOOL_LIST.items()])

    def get_tool(self, tool_name: str):
        if tool_name not in self.TOOL_LIST:
            raise ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_NAME.name,
    ...
    def analyze_git_history(self, file_path: str, commit_range: str = "HEAD~5..HEAD") -> str:
        ...
        try:
            result = subprocess.run(
                ["git", "log", commit_range, "--pretty=format:%H%n%an%n%ad%n%s%n%b", "--", file_path],
                capture_output=True, text=True, check=True)
...
"""
    ...
    def save_file(self, file_path: str, content: str) -> str:
        ...
        if self.is_test_file(file_path) or "reproduce" in file_path.lower():
            raise ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
    ...
    def get_function_body(self, file_path: str, function_name: str) -> str:
        ...
        try:
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.FILE_NOT_FOUND.name,
                    ...
                    if isinstance(node, ast.ClassDef) and node.name == class_name:
                        ...
                        for class_child in node.body:
                            if isinstance(class_child,
                                          (ast.FunctionDef, ast.AsyncFunctionDef)) and class_child.name == method_name:
    ...
    def get_git_log(self, num_commits: int = 10) -> str:
        ...
        '''
        try:
            result = subprocess.run(["git", "log", f"-{num_commits}", "--oneline", "--graph"], capture_output=True,
    ...
    def get_function_ranges(self, file_path: str) -> list[tuple[int, int, str]]:
        ...
        try:
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.SYNTAX_ERROR.name,
    ...
    def search_in_specified_file_v2(self, file_path: str, search_term: str) -> str:
        ...
        if not file_path.endswith(".py"):
            raise ToolManager.Error(ToolManager.Error.ErrorType.INVALID_FILE_PATH.name,
    ...
    def search_recurive_in_all_files_in_directory(self, directory_path: str, search_term: str) -> str:
        ...
        if not output:
            raise ToolManager.Error(ToolManager.Error.ErrorType.SEARCH_TERM_NOT_FOUND.name,
                                    f"'{search_term}' not found in file '{directory_path}'")
        return output
    ...
    def apply_code_edit(self, file_path: str, search: str, replace: str) -> str:
        ...
        match original.count(search):
            case 0:
                logger.error(
                    f"search string not found in file {file_path}. You need to share the exact code you want to replace.")
                raise ToolManager.Error(ToolManager.Error.ErrorType.SEARCH_TERM_NOT_FOUND.name,
            ...
            case num_hits:
                logger.error(
                    f"search string found {num_hits} times in file '{file_path}'.\nPlease reformulate your search and replace to apply only one change.")
                raise ToolManager.Error(ToolManager.Error.ErrorType.MULTIPLE_SEARCH_RESULTS_FOUND.name,
    ...
    def test_patch_find_finish(self, test_func_names: List[str]):
        ...
        for test_func_name in test_func_names:
            if " - " not in test_func_name:
                return ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                                         f"Error: test_func_name '{test_func_name}' is not in the correct format. Each string should be in the format 'test_file_path.py - test_func_name'.")
            ...
            if not function_name:
                return ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                                         f"Error: function name is empty in '{test_func_name}'.")
            if not file_path.endswith(".py"):
                return ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                                         f"Error: file '{file_path}' is not a Python file.")
            if not function_name.isidentifier():
                return ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                                         f"Error: function name '{function_name}' is not a valid Python function name.")
        return "finish"
    ...
    def save_relevant_test(self, test_func_name: str):
        ...
        relevant_nodeids = self.find_relevant_tests(failed_and_error_nodeids, problem_statement) if len(
            failed_and_error_nodeids) > 5 else failed_and_error_nodeids
    ...
    def find_relevant_tests(self, test_func_names: List[str], problem_statement: str):
        ...
        async def process_test_case(test_case):
            ...
            instance_prompt = self.INSTANCE_TEST_CASES_PROMPT_TEMPLATE.format(problem_statement=problem_statement,
    ...
    def compile_repo(self) -> str:
        ...
        '''
        try:
            ls = subprocess.run(["bash", "-c", "git ls-files '*.py'; ls -1 **/*.py 2>/dev/null | cat"],
    ...
    def list_python_files(self) -> str:
        ...
        '''
        try:
            res = subprocess.run(["bash", "-c", "git ls-files '*.py'; ls -1 **/*.py 2>/dev/null | cat"],
                                 capture_output=True, text=True)
    ...
    @tool
    def finish(self, run_repo_tests_passed: bool, run_repo_test_depdency_error: bool,
        ...
        try:
            if not run_repo_tests_passed and not run_repo_test_depdency_error:
                raise ToolManager.Error(ToolManager.Error.ErrorType.BUG_REPORT_REQUIRED.name,
                                        f"Error: tests failed. Please fix the issue before you can finish the task")
...
"""
    ...
    def search_in_all_files_content_v3(self, grep_search_command: str) -> str:
        ...
        if not output:
            raise ToolManager.Error(ToolManager.Error.ErrorType.SEARCH_TERM_NOT_FOUND.name,
                                    f"'{grep_search_command}' not found.")
...
{"status": "STATUS", "details": "specific explanation"}"""
            ...
            loop = asyncio.get_event_loop()
            with ThreadPoolExecutor() as executor:
                validation_result = await loop.run_in_executor(executor, Network._request_next_action_with_retry,
                                                               messages)
        ...
        need_more_files = [result for result in failed_tests if
                           "CURRENT CHANGES DON'T DIRECTLY AFFECT THE TEST FUNCTION. CHECK BASE, DERIVED, OR RELATED FILES" in result]
        ...
        if "::" in test_name:
            ...
            if inner_match:
                ...
                if "FAILED" in inner_session or "FAILURES" in inner_session:
                    ...
                    inner_failures_match = re.search(r'={5,}\s*FAILURES\s*={5,}.*?(?=={5,}|\Z)', inner_session,
                    ...
                    if inner_failures_match:
                        ...
                        if len(inner_failures) > 3000:
                            ...
                            if len(lines) > 100:
                                inner_failures = "\n".join(
                                    lines[:50] + [f"... (truncated {len(lines) - 100} lines) ..."] + lines[-50:])
...
"""
            ...
            if test_runner != 'pytest' and self._check_dependency_errors(output):
                ...
                if test_runner_mode == "MODULE":
                    ...
                    if len(out) > 20000:
                        ...
                        if len(lines) > 500:
                            output = "\n".join(
                                lines[:400] + ["... ({} lines omitted) ...".format(len(lines) - 500)] + lines[-100:])
                        else:
                            output = out
                    else:
                        output = out
    ...
    def apply_code_edit(self, file_path: str, search: str, replace: str) -> str:
        ...
        if self.is_test_file(file_path) and "pytest" not in file_path.lower():
            raise ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_CALL.name,
                                    f"Error: You cannot create or change test files. Try another way.")
        if not os.path.exists(file_path):
            logger.error(f"file '{file_path}' does not exist.")
            raise ToolManager.Error(ToolManager.Error.ErrorType.FILE_NOT_FOUND.name,
                                    f"Error: file '{file_path}' does not exist.")
    ...
    def filter_test_func_names(self, test_file_paths: List[str]):
        ...
        if "Successfully ran all tests" not in output:
            if self._check_dependency_errors(
                    output) or self.is_repeating > 3:  # we can't use run_repo_tests when there is dependency errors
    ...
    def run_repo_tests(self, timeout_secs: int = 135) -> str:
        ...
        if self.failed_test_names is None:
            if self.first_run_repo_tests_call:  # try to run tests on directories first
                ...
                for test_file in self.test_files:
                    ...
                    if test_dir:  # Only add non-empty directory paths
                        try:
                            if len([f for f in os.listdir(test_dir) if
                                    os.path.isfile(os.path.join(test_dir, f))]) >= 30:
            ...
            if len(failed_test_names) > 10:  # if there are too many failures in that directory, just try to run the file because there is time limit.
                ...
                self.logs.append(
                    f"There are too many failures in that directory, running tests on the specified files only.")
            ...
            if "ERROR: tests timed out." in output:
                ...
                self.logs.append(
                    f"Running tests on the full directory timedout, running test on the specified files only.")
                output, result = self._run_repo_tests_with_timeout(list(self.test_files), timeout_secs=timeout_secs)
                if result:
                    self.can_finish = True
                    return output
            ...
            if self._count_modified_or_added_lines_from_patch(
                    current_patch) < 5:  # changes are small, so might not need to check other test functions
                print(
                    "Successfully run on failed tests, running on all tests again., Changes are small, so skip checking other test functions.")
                self.logs.append(
                    f"Successfully run on failed tests, running on all tests again., Changes are small, so skip checking other test functions.")
                self.can_finish = True
                return output
            else:
                print(
                    f"Successfully run on failed tests, running on all tests again., Changes are large, so checking other test functions to be sure that I didn't break other tests.")
                self.logs.append(
                    f"Successfully run on failed tests, running on all tests again., Changes are large, so checking other test functions to be sure that I didn't break other tests.")
...
def has_dependency_error_task_process(input_dict: Dict[str, Any], repod_dir: str = 'repo'):
    ...
    return {"patch": patch_text, "test_func_names": list(test_func_names), "logs": "", "test_patch_find_messages": [],
            "patch_find_messages": [], "elapsed_time": time.time() - workflow_start_time,
            "type": "pytest_not_available"}
...
def execute_test_patch_find_workflow_v0(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "",
                                        search_results: str = "", hints: str = "") -> tuple[List[str], List[str]]:
    global run_id
    run_id = run_id_1
    ...
    system_prompt = TEST_PATCH_FIND_SYSTEM_PROMPT_TEMPLATE_V0.format(tools_docs=ToolManager.get_tool_docs(),
    ...
    for step in range(MAX_STEPS_TEST_PATCH_FIND):
        ...
        if time.time() - start_time > timeout:
            cot.add_action(
                COT.Action(next_thought="global timeout reached", next_tool_name="", next_tool_args={}, observation="",
                           is_error=True, inference_error_counter={}, request_data=[]))
            break
        ...
        if cot.is_thought_repeated():
            ...
            messages.append({"role": "user", "content": DO_NOT_REPEAT_TOOL_CALLS.format(
        ...
        try:
            ...
            cot.add_action(
                COT.Action(next_thought=error_msg, next_tool_name="", next_tool_args={}, observation="", is_error=True,
                           raw_response=raw_text, total_attempts=total_attempts, inference_error_counter=error_counter,
        ...
        try:
            logger.info(
                f"[TEST_PATCH_FIND] next_thought: {next_thought}\nnext_tool_name: {next_tool_name}\nnext_tool_args: {next_tool_args}\n")
            if '"' in next_tool_name or "'" in next_tool_name:
                next_tool_name = next_tool_name.replace('"', '')
                next_tool_name = next_tool_name.replace("'", "")

            next_observation = tool_manager.get_tool(next_tool_name)(
                **next_tool_args) if next_tool_args else tool_manager.get_tool(next_tool_name)()
            ...
            if next_tool_name == "find_relevant_tests_in_file" and next_observation == []:
                cot.add_action(COT.Action(
                    next_thought="[TEST_PATCH_FIND] [CRITICAL] No relevant tests found. Need to try again from the start. Find another relevant test files and call find_relevant_tests_in_file. Start from the beginning.",
                    next_tool_name=next_tool_name, next_tool_args=next_tool_args,
                    observation="[TEST_PATCH_FIND] [CRITICAL] No relevant tests found. Need to try again from the start. Find another relevant test files and call find_relevant_tests_in_file. Start from the beginning.",
                    is_error=False, raw_response=raw_text, total_attempts=total_attempts,
                    inference_error_counter=error_counter, request_data=messages))
                continue
            cot.add_action(
                COT.Action(next_thought=next_thought, next_tool_name=next_tool_name, next_tool_args=next_tool_args,
                           observation=next_observation, is_error=False, raw_response=raw_text,
                           total_attempts=total_attempts, inference_error_counter=error_counter, request_data=messages))
        except ToolManager.Error as e:
            import traceback  # Ensure traceback is accessible
            ...
            cot.add_action(
                COT.Action(next_thought=next_thought, next_tool_name=next_tool_name, next_tool_args=next_tool_args,
                           observation=error_msg, is_error=True, raw_response=raw_text, total_attempts=total_attempts,
                           inference_error_counter=error_counter, request_data=messages))
            continue
        except Exception as e:
            import traceback  # Ensure traceback is accessible
        ...
        if next_tool_name == "test_patch_find_finish_v0":
            ...
            if len(test_func_names) > 0:
                logger.info(
                    f'[TEST_PATCH_FIND] [CRITICAL] Workflow called test_patch_find_finish_v0 operation with test_func_names: {test_func_names}')
                logs.append(
                    f"Workflow called test_patch_find_finish_v0 operation with test_func_names: {test_func_names}\n\n")
        ...
        cot.add_action(
            COT.Action(next_thought="global timeout reached", next_tool_name="", next_tool_args={}, observation="",
                       is_error=True))
    logger.info(
        f"[TEST_PATCH_FIND] [CRITICAL] Workflow completed after reaching MAX_STEPS ({MAX_STEPS_TEST_PATCH_FIND}) without finding relevant tests")
    logs.append(f"Workflow reached MAX_STEPS without finding relevant tests\n\n")
    return tool_manager.RELEVANT_TEST_FUNC_NAMES, logs


def execute_fix_workflow_v0(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "",
                            test_func_codes: List[tuple[str, str, str]] = None, test_file_paths: List[str] = None) -> \
tuple[str, List[str], List[str]]:
    ...
    system_prompt = FIX_SYSTEM_PROMPT_TEMPLATE_V0.format(tools_docs=ToolManager.get_tool_docs(),
                                                         format_prompt=FORMAT_PROMPT_V0)
    ...
    logger.info(
        f"Starting workflow execution with {MAX_STEPS} max steps: timeout: {timeout} seconds : run_id: {run_id}")
...
def execute_workflow_with_no_pytest(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "",
                                    test_func_code: List[str] = None, test_file_paths: List[str] = None) -> tuple[
    str, List[str]]:
    global run_id
    run_id = run_id_1
    ...
    for step in range(MAX_STEPS):
        ...
        messages.append({"role": "system", "content": STOP_INSTRUCTION})
        if (
                last_previous_next_tool_name == previous_next_tool_name and last_previous_next_tool_args == previous_next_tool_args):
            logger.info(f"Thought repeated, adding DO NOT REPEAT TOOL CALLS instruction")
            messages.append({"role": "user", "content": DO_NOT_REPEAT_TOOL_CALLS.format(
                previous_response=f"next_tool_name:{last_previous_next_tool_name}\n next_tool_args:{last_previous_next_tool_args}")})
...
def execute_agent_workflow(
    ...
    for step in range(max_steps):
        logger.info(
            f"[{log_prefix}] Execution step {step + 1}/{max_steps}, Elapsed time: {time.time() - start_time} seconds, timeout: {timeout} seconds")
        logs.append(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] [{log_prefix}] Execution step {step + 1}/{max_steps}, Elapsed time: {time.time() - start_time} seconds, timeout: {timeout} seconds\n\n")
        model_upgrade = False
        start_over = False
        ...
        if start_over:
            ...
            messages.append({"role": "user",
                             "content": "Summarize what you've tried until now using `summarize_what_you_tried` tool. DO NOT USE ANY OTHER TOOLS."})
            next_thought, next_tool_name, next_tool_args, raw_text, total_attempts, error_counter, messages = Network.inference(
                messages, model=current_model, run_id=run_id)
            summarization_tool_name = "summarize_what_you_tried"
            if isinstance(next_tool_name, str) and next_tool_name == summarization_tool_name or (
                    isinstance(next_tool_name, list) and summarization_tool_name in next_tool_name):
                if isinstance(next_tool_name, list):
                    next_tool_args = next_tool_args[next_tool_name.index(summarization_tool_name)]

                logger.info(
                    f"[{log_prefix}] Summarizing what you've tried until now and starting over: {next_tool_args['summarization']}")
                logs.append(
                    f"[{log_prefix}] Summarizing what you've tried until now and starting over: {next_tool_args['summarization']}\n\n")
                ...
                logger.info(
                    f"[{log_prefix}] summarization tool call failed: {next_tool_name} called instead of summarize_what_you_tried")
                logs.append(
                    f"[{log_prefix}] summarization tool call failed: {next_tool_name} called instead of summarize_what_you_tried\n\n")
                messages[-1] = {"role": "user",
                                "content": "Please start over the process again using `start_over` tool since you're stuck."}
        ...
        if blacklisted_test_files and len(blacklisted_test_files) > 0:
            messages.append({"role": "user",
                             "content": f"AS A REMINDER, DO NOT SEARCH OR USE THESE FILES:\n\n{blacklisted_test_files}"})

        if time.time() - start_time > timeout - warning_time_limit:
            messages.append({"role": "user",
                             "content": f"YOU'RE RUNNING OUT OF TIME, PLEASE FINISH THE WHOLE PROCESS IN {timeout - time.time() + start_time} SECONDS."})
        ...
        try:
            ...
            formatted_observation = '\n\n'.join(next_observation) if isinstance(next_observation, list) else str(
                next_observation)
        ...
        if (isinstance(next_tool_name, str) and next_tool_name == finish_tool_name) or (
                isinstance(next_tool_name, list) and finish_tool_name in next_tool_name):
            if isinstance(next_tool_name, list):
            ...
            if finish_tool_name == "test_patch_find_finish":
                if next_observation == "finish":
                    logger.info(
                        f'[{log_prefix}] [CRITICAL] Workflow called {finish_tool_name} operation with test_func_names: {tool_manager.filtered_test_func_names}')
                    logs.append(
                        f"[{log_prefix}] Workflow called {finish_tool_name} operation with test_func_names: {tool_manager.filtered_test_func_names}\n\n")
...
def execute_test_patch_find_workflow_v1(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "",
                                        search_results: str = "", hints: str = "") -> tuple[List[str], List[str]]:
...
def execute_fix_workflow_v1(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "",
                            test_file_paths: List[str] = None) -> tuple[List[str], List[str]]:
...
def multi_task_process(input_dict: Dict[str, Any], repod_dir: str = 'repo'):
    ...
    print(f"[CRITICAL] task processor returning patch length: {len(patch_text)}")
    return {"patch": patch_text, "test_func_names": "", "logs": "", "test_patch_find_messages": "",
            "patch_find_messages": "", "elapsed_time": time.time() - workflow_start_time, "type": "pytest_available"}